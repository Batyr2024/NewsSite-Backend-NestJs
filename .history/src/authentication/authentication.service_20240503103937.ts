...
export class AuthenticationService {
    ...
    public async getAuthenticatedUser(login: string, hashPassword: string) {
        try {
            const user = await this.userService.getByLogin(login);
            const isPasswordMatching = await bcrypt.compare(hashPassword, user.password);
    ...
    private verifyPassword(plainTextPassword)
}
